...
def dakai():
    ...
    # 读取sql文件并生成dateframe
    with open('./音乐T-1打开.sql', encoding='utf-8') as f:
        dakai_sql = f.read()
    df = pd.read_sql(dakai_sql, con)
    ...
    # 设置标题
    ax.set_title("音乐T-1各活动打开效果", loc='center', fontweight='bold', fontdict={'size': 25})
    ...
    # 返回最佳打开相关字段
    best_open_name = df['专题名称(活动名称）'][0]
    best_open_rate = df['打开率'][0]
    return best_open_name, best_open_rate
