...
def laxin():
    ...
    df = pd.read_excel(path, sheet_name='阅读t-2拉新图', usecols=[0, 1, 2])
    # df = pd.read_excel(r'D:\0刘宇\日报\202306\0605\阅读数据表.xlsx', sheet_name='阅读t-2拉新图', usecols=[0, 1, 2])
    ...
    # 折线图
    plt.plot(df['专题名称(活动名称）'], df['促活(含拉新)率'], color='coral', marker='o', linewidth=3)

    # 给折线图添加数据标签
    for a, b in zip(df['专题名称(活动名称）'], df['促活(含拉新)率']):
        plt.text(a, b, '{:.3%}'.format(b), ha='left', va='baseline', fontsize=16)
    plt.ylim(0)
    plt.axis('off')

    # 设置标题
    ax.set_title("阅读T-2各活动用户发展效果", loc='center', fontweight='bold',  fontdict={'size': 25})

    plt.subplots_adjust(bottom=0.2)

    # table图表
    celltext = [df['覆盖量(万)'].map(lambda x: round(x/10000, 1)).values.tolist(),
                df['促活(含拉新)率'].map(lambda x: '{:.3%}'.format(x)).values.tolist()]
    rowlabels = ['覆盖量(万)', '促活(含拉新)率']
